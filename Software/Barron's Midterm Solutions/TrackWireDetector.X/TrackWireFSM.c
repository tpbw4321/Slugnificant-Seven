...
uint8_t InitTrackWireFSM(uint8_t Priority) {
    MyPriority = Priority;
 ...
 *        Returns TRUE if successful, FALSE otherwise
 * @author J. Edward Carryer, 2011.10.23 19:25 */
uint8_t PostTrackWireFSM(ES_Event ThisEvent) {
 ...
 * @author J. Edward Carryer, 2011.10.23 19:25 */
ES_Event RunTrackWireFSM(ES_Event ThisEvent) {
    ...
    switch (CurrentState) {
        ...
        case Searching: // in the first state, replace this with appropriate state
            switch (ThisEvent.EventType) {
                case ES_NO_EVENT:
                    //Search for TrackWire using Param.
                    break;
                case TRACK_WIRE_NEARBY:
                    nextState = TrackWireNearby;
                    makeTransition = TRUE;
                    ThisEvent.EventType = ES_NO_EVENT;
                    break;
                default:
        ...
        case TrackWireNearby:
            switch (ThisEvent.EventType) {
                case ES_NO_EVENT:
                    ...
                    ThisEvent.EventType = ES_NO_EVENT;
                    break;
                case CLOSEST_TO_TRACKWIRE:
                    nextState = ClosestToTrackWire;
                    makeTransition = TRUE;
                    ThisEvent.EventType = ES_NO_EVENT;
                    break;
                case TRACK_WIRE_NEARBY:
                    //Do some searching with PARAM for tracking
                    break;
                default:
                    break;
            }
            break;
        case ClosestToTrackWire:
            switch (ThisEvent.EventType) {
                case ES_NO_EVENT:
                    nextState = Searching;
                    makeTransition = TRUE;
                    ...
                    break;
                case CLOSEST_TO_TRACKWIRE:
                    //Hide and increment disabled obstacles
                    break;
                default:
                    break;
            }
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunTrackWireFSM(EXIT_EVENT);
        CurrentState = nextState;
        RunTrackWireFSM(ENTRY_EVENT);
    }

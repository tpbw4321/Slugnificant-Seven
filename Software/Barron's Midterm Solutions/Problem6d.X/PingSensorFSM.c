    ...
    switch (CurrentState) {
        ...
        case ProcessDistance:
            switch (ThisEvent.EventType) {
                case ES_TIMEOUT:
                    if (!ECHO) {
                        timeElapsed = timeStamp = TIMERS_GetTime();
                        distanceInInches = (timeElapsed * MILI_TO_MICRO_SCALAR) / INCHES_CONVERSION_RATE;

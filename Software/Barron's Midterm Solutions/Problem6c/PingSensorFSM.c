...
//#include "PingSensorTimerService.h"
#include <BOARD.h>
#include "xc.h"
#include "timers.h"
#include "IO_Ports.h"
...
#define ECHO  PORTZ04_BIT 
#define TRIG  PORTZ03_LAT
...
static uint16_t distanceInInches = 0;
static uint8_t sensorDataReady = FALSE;
...
uint8_t InitPingSensorFSM(uint8_t Priority) {
    MyPriority = Priority;
 ...
 *        Returns TRUE if successful, FALSE otherwise
 * @author J. Edward Carryer, 2011.10.23 19:25 */
uint8_t PostPingSensorFSM(ES_Event ThisEvent) {
 ...
 * @author J. Edward Carryer, 2011.10.23 19:25 */
ES_Event RunPingSensorFSM(ES_Event ThisEvent) {
    uint8_t makeTransition = FALSE; // use to flag transition
    static uint16_t timeStamp = 0;
    static uint16_t timeOutTimeStamp = 0;
    ...
    switch (CurrentState) {
            currTime = TIMERS_GetMicroSeconds();
        ...
        case AwaitingCommand:
            ...
            break;
        case SendPulse: // in the first state, replace this with appropriate state
            switch (ThisEvent.EventType) {
                case ES_ENTRY:
                    TRIG = 1;
                    timeStamp = currTime;
                ...
                case ES_TIMEOUT:
                    if (timeStamp - currTime != 0) {
                        TRIG = 0;
                        nextState = DetectEcho;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
                    if (timeOutTimeStamp - currTime > SENSOR_TIMEOUT) {
                        nextState = AwaitingCommand;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                        distanceInInches = INF;
                    }
                    break;
                default:
                    break;
        ...
        case ProcessDistance:
            switch (ThisEvent.EventType) {
                case ES_TIMEOUT:
                    if (!ECHO) {
                        timeElapsed = timeStamp - TIMERS_GetMicroSeconds();
                        distanceInInches =  timeElapsed / INCHES_CONVERSION_RATE;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunPingSensorFSM(EXIT_EVENT);
        CurrentState = nextState;
        RunPingSensorFSM(ENTRY_EVENT);
    }
    ES_Tail(); // trace call stack end
    return ThisEvent;
}

//Checks to see if the current data is ready
//Returns TRUE or FALSE
uint8_t IsNewPingSensorDataReady(){
    return sensorDataReady;}

//Returns the distance if a measurement is not being taken
//if data is not ready INF is returned.
uint16_t GetDistanceInInches(){
    if(IsNewPingSensorDataReady())

...
#define TAPE_SERVICE_TIMER_TICKS 2 //2ms
...
uint8_t InitTapeSensorService(uint8_t Priority) {
    ...
    printf("Tape Sensor\r\n");
    if(TIMERS_InitTimer(TAPE_SENSOR_SERVICE_TIMER, TAPE_SERVICE_TIMER_TICKS) == ERROR){
...
ES_Event RunTapeSensorService(ES_Event ThisEvent) {
    ...
    switch (ThisEvent.EventType) {
        ...
        case ES_TIMEOUT:
            ...
            ReturnEvent.EventParam = TAPE_SENSOR_SERVICE_TIMER;
            lastEvent = curEvent; // update history
#ifndef SIMPLESERVICE_TEST           // keep this as is for test harness
            RunMainStateMachineHSM(ReturnEvent);
#else

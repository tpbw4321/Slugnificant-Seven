    ...
    switch (CurrentState) {
        case InitPSubState: // If current state is initial Psedudo State
            if (ThisEvent.EventType == ES_INIT)// only respond to ES_Init
            {
                ...
                nextState = GO_STRAIGHT; //SHIMY_LEFT 
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            }
            break;
        case SHIMY_LEFT:
            robotspeed(SHIMMY_SPEED, ROBOT_SPEED);                                                                                                                                                 
            switch (ThisEvent.EventType) {
                case ES_NO_EVENT:
                    break;
                case ON_TAPE:
                case OFF_TAPE:
                    if ((ThisEvent.EventParam & TR_TAPE)) {
                        nextState = SHIMY_RIGHT;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    } else if ((ThisEvent.EventParam == TC_TAPE) || ThisEvent.EventParam == (TC_TAPE | BC_TAPE)) {
            ...
            break;
        case SHIMY_RIGHT:
            robotspeed(ROBOT_SPEED, SHIMMY_SPEED); 
            switch (ThisEvent.EventType) {
                case ES_NO_EVENT:
                    break;
                case ON_TAPE:
                case OFF_TAPE:
                    if ((ThisEvent.EventParam & TL_TAPE)) {
                        nextState = SHIMY_LEFT;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    } else if ((ThisEvent.EventParam == TC_TAPE) || ThisEvent.EventParam == (TC_TAPE | BC_TAPE)) {
                        nextState = GO_STRAIGHT;
            ...
            switch (ThisEvent.EventType) {
                ...
                case OFF_TAPE:
                    if ((ThisEvent.EventParam & TL_TAPE)) {
                        ...
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    } else if ((ThisEvent.EventParam & TR_TAPE)) {
                        nextState = SHIMY_RIGHT;
                        makeTransition = TRUE;
                        ThisEvent.EventType = ES_NO_EVENT;
                    }
